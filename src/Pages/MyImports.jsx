...
const MyImports = () => {
  ...
  const handleRemove = async (id) => {
    ...
    try {
      ...
      toast.success("Import removed and stock restored!");
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 px-4">
      ...
      {importsData.length === 0 ? (
        <p>You haven’t imported any products yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {importsData.map((item) => (
            <div key={item._id} className="border rounded-lg p-4 shadow hover:shadow-lg flex flex-col">
              <img
                src={item.productDetails.image || "https://via.placeholder.com/150"}
                alt={item.productDetails.name}
                className="h-40 w-full object-cover rounded"
              />
              <h3 className="font-bold mt-2">{item.productDetails.name}</h3>
              <p>Price: {item.productDetails.price.toFixed(2)} TK</p>
              <p>Origin: {item.productDetails.originCountry || "N/A"}</p>
              <p>Rating: {item.productDetails.rating || 0}</p>
              <p>Imported Quantity: {item.quantity}</p>

              <div className="mt-4 flex gap-2">
                <Link
                  to={`/products/${item.productDetails._id}`}
